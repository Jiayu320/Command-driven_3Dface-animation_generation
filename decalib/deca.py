...
class DECA(nn.Module):
    ...
    def encode(self, images, use_detail=True):
        ...
        codedict['neck_pose'] = torch.tensor([[0., 0., 0.]], dtype=torch.float32)  
        return codedict
    
    
    # @torch.no_grad()
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,render_orig=False, original_image=None, tform=None):
        images = codedict['images']
        batch_size = images.shape[0]
        
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'], neck_pose=codedict['neck_pose'])
    ...
    def save_obj(self, filename, opdict):
        ...
        verts, uvcoords, faces, uvfaces = util.load_obj('TestSamples/examples/results/me/me_detail.obj')
        return verts.numpy(), dense_colors, faces.numpy()
