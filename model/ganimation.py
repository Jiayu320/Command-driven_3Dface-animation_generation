...
class GANimationModel(BaseModel):
    ...
    def feed_batch(self, batch):
        self.src_FLAMEpara = batch['src_FLAMEpara'].to(self.device)
        ...
        if self.is_train:
            ...
            self.tar_FLAMEpara = batch['tar_FLAMEpara'].to(self.device)

    def forward(self):
        # generate fake image
        self.color_mask ,self.aus_mask, self.embed = self.net_gen(self.src_FLAMEpara, self.tar_aus)
        self.fake_img = self.aus_mask * self.src_FLAMEpara + (1 - self.aus_mask) * self.color_mask
    ...
    def backward_dis(self):
        # real image
        pred_real, self.pred_real_aus = self.net_dis(self.src_FLAMEpara)
        ...
        if self.opt.gan_type == 'wgan-gp':
            self.loss_dis_gp = self.gradient_penalty(self.src_FLAMEpara, self.fake_img)
    ...
    def backward_gen(self):
        ...
        self.loss_gen_rec = self.criterionL1(self.rec_real_img, self.src_FLAMEpara)
    ...
    def get_latest_visuals(self):
        visuals_name = ['src_FLAMEpara', 'tar_FLAMEpara', 'color_mask', 'aus_mask', 'fake_img']
        if self.is_train:
